...
namespace CsvLibraryTest
{
    public class UnitTest1
    {
        ...
        [Fact]
        public void Test_FromFile_DoubleQuatation_WithComma()
        {

            var reader = new CsvReader();
            //var s = "“c’†,21,“Œ‹“s`‹æ" + Environment.NewLine + "—é–Ø,22,“Œ‹“sVh‹æ";           
            var filePath = System.IO.Path.Combine(Environment.CurrentDirectory, "Csv"
                , "CsvSample_DoubleQuatation_WithComma.csv");
            var bodyText = File.ReadAllText(filePath);
            var rowList = reader.Read(bodyText);
        ...
        [Fact]
        public void Test_FromFile_DoubleQuatation_WithDoubleQuatation()
        {

            var reader = new CsvReader();
            //var s = "“c’†,21,“Œ‹“s`‹æ" + Environment.NewLine + "—é–Ø,22,“Œ‹“sVh‹æ";           
            var filePath = System.IO.Path.Combine(Environment.CurrentDirectory, "Csv"
                , "CsvSample_DoubleQuatation_WithDoubleQuatation.csv");
            var bodyText = File.ReadAllText(filePath);
            var rowList = reader.Read(bodyText);
